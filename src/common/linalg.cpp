...
namespace linalg
{
	namespace Matrix
	{
		...
		d_Tensor::Tensor Transpose(d_Tensor::Tensor& A, bool inplace)
		{
	...
	namespace Vector
	{
		...
		double dot(const d_Tensor::Tensor& A, const d_Tensor::Tensor& B)
		{
			...
			{
				throw("make sure both are tensors with 1 axis");
			}

			if (A.shape() != B.shape())
			{
				throw("please enter vectors of equal dimensions");
			}
